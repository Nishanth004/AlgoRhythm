...
# Generator.py
def generate_sequence(model, seed, num_steps, output_units, device, temperature=1.0):
    model.eval()
    ...
    for _ in range(num_steps):
        with torch.no_grad():
            output = model(seed)  # shape: (batch, output_units)

            # Apply temperature before softmax
            temp = max(float(temperature), 1e-6)
            logits = output / temp
            probabilities = torch.softmax(logits, dim=1).cpu().data.numpy()
            ...
            next_input = torch.nn.functional.one_hot(
                next_input, num_classes=output_units
            ).float()
